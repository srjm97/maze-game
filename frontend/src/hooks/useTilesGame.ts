...
import { TILES_CONFIG, TILE_COLORS, TILE_SYMBOLS } from '../constants/tilesConstants';
...
export const useTilesGame = () => {
  ...
  const createTiles = useCallback((width: number, height: number): TileData[] => {
   ...
   */
  const handleTileClick = useCallback((index: number): boolean => {
    ...
    setTiles(prev => prev.map((t, i) => (i === index ? { ...t, isRevealed: true } : t)));
